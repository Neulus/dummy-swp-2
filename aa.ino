...
#define TIMEOUT ((INTERVAL / 2) * 1000.0) - 100// maximum echo waiting time (unit: usec)
...
void loop() { 
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      analogWrite(PIN_LED, (int) (((float) (100 - abs(distance - 200))) / 100.0 * 255.0));       // LED ON 
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  int pulseTime = pulseIn(ECHO, HIGH, TIMEOUT);
  // Serial.print("TIME: "); Serial.println(pulseTime);
  if (pulseTime == 0) {
    pulseTime = latest_sane;
  } else {
    latest_sane = pulseTime;
  }
  return pulseTime * SCALE; // unit: mm
