...
void loop() {
  ...
  // add your code here!
  float distance_percentage = ((float) (dist_ema - _TARGET_LOW)) / (_TARGET_HIGH - _TARGET_LOW);
  int servo_duty = -(distance_percentage < 0.5) & (_DUTY_MIN + (int) ((float) (_DUTY_NEU - _DUTY_MIN) * (distance_percentage * 2.0))) | -(!(distance_percentage < 0.5)) & (_DUTY_NEU + (int) ((float) (_DUTY_MAX - _DUTY_NEU) * (distance_percentage - 0.5) * 2.0));
